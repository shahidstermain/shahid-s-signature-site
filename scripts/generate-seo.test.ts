...
describe("generate-seo script", () => {
  ...
  describe("error handling and resilience", () => {
    it("should handle writeFile errors for specific files", async () => {
      const { generateRSSFeed, generateJSONFeed } = await import("../src/lib/rss");
      const { generateSitemap, generateRobotsTxt } = await import("../src/lib/sitemap");

      // Mock writeFile to fail on second call
      let callCount = 0;
      ...
      const writePromises = outputs.map((output) =>
        writeFile(path.join(process.cwd(), "public", output.name), output.contents, "utf8").catch((e) => e)
      );
    ...
    it("should validate output content before writing", async () => {
      const { generateRSSFeed, generateJSONFeed } = await import("../src/lib/rss");
      const { generateSitemap } = await import("../src/lib/sitemap");
      ...
      // Sitemap and RSS must contain XML declaration
      expect(sitemap).toMatch(/^<\?xml/);
      expect(rss).toMatch(/^<\?xml/);

      // JSON feed must be valid JSON
      expect(() => JSON.parse(feed)).not.toThrow();
    ...
    it("should handle concurrent write operations", async () => {
      ...
      const writes = Array(5)
        .fill(null)
        .map(() => writeFile(filePath, content, "utf8"));
    ...
    it("should generate consistent output structure", async () => {
      ...
      // Should contain same article URLs
      expect(sitemap1).toContain("article-1");
      expect(sitemap2).toContain("article-1");
      expect(sitemap1).toContain("article-2");
      expect(sitemap2).toContain("article-2");
    });

    it("should handle empty mocked generation functions", async () => {
      // Override mocks to return minimal output
      const { generateSitemap } = await import("../src/lib/sitemap");

      const sitemap = generateSitemap();

      // Even with mocked data, should produce valid XML
      expect(sitemap).toMatch(/^<\?xml/);
      expect(sitemap).toContain("</urlset>");
    });
  });

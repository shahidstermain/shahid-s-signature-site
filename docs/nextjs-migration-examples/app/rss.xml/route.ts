...
/**
 * Create a plain-text excerpt from Markdown or HTML suitable for summaries.
 *
 * @param content - Markdown or HTML input to convert into a plain-text summary.
 * @returns Plain text where code blocks and inline code are removed, basic formatting and headers are stripped, links are converted to their link text, whitespace is collapsed and trimmed, and the result is truncated to 500 characters.
 */
...
/**
 * Produce the site's RSS 2.0 XML feed containing all blog articles.
 *
 * The feed includes channel metadata and one item per article; textual fields are XML-escaped
 * and items are ordered newest-first.
 *
 * @returns An HTTP Response whose body is the RSS XML document and that includes Content-Type and cache-related headers.
 */

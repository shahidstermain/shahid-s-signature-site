...
/**
 * Generate sitemap entries for the site including the homepage and all blog posts.
 *
 * The homepage entry uses the current date, a weekly `changeFrequency`, and priority `1.0`.
 * Blog entries are generated at `/blog/{slug}` with `lastModified` derived from the article's `date`, `changeFrequency` set to `monthly`, and priority `0.9` for featured posts or `0.8` otherwise. Blog posts are sorted by `lastModified` in descending order (newest first).
 *
 * @returns The array of sitemap entries with the homepage entry first followed by the sorted blog post entries.
 */

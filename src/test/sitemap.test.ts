...
`;
  ...
  describe("Performance and Stress Tests", () => {
    it("should handle large number of articles efficiently", () => {
      ...
      vi.spyOn(articlesModule, "articles", "get").mockReturnValue(manyArticles as any);
    ...
    it("should generate valid XML even with unusual slugs", () => {
      const articlesWithUnusualSlugs = [
        { ...mockArticles[0], slug: "test---multiple---dashes" },
        { ...mockArticles[0], slug: "test_underscores_here" },
        { ...mockArticles[0], slug: "test.with.dots" },
      ];

      vi.spyOn(articlesModule, "articles", "get").mockReturnValue(articlesWithUnusualSlugs as any);

      const sitemap = generateSitemap();
      expect(sitemap).toContain("test---multiple---dashes");
      expect(sitemap).toContain("test_underscores_here");
      expect(sitemap).toContain("test.with.dots");
    });

    it("should maintain consistent XML structure with edge case dates", () => {
      const articlesWithEdgeDates = [
        { ...mockArticles[0], slug: "old", date: "Jan 1900" },
        { ...mockArticles[0], slug: "future", date: "Dec 2099" },
      ];

      vi.spyOn(articlesModule, "articles", "get").mockReturnValue(articlesWithEdgeDates as any);

      const sitemap = generateSitemap();
      expect(sitemap).toContain("<lastmod>1900-01-01</lastmod>");
      expect(sitemap).toContain("<lastmod>2099-12-01</lastmod>");
    });
  });
});
...
describe('Sitemap Generation', () => {
  ...
  describe('Negative Cases', () => {
    it('should handle invalid date formats gracefully', () => {
      ...
      const sitemap = generateSitemap(invalidDateArticles);
      expect(sitemap).toHaveLength(2); // homepage + article
    ...
    it('should handle null/undefined values', () => {
      ...
      const sitemap = generateSitemap(nullArticles);
      expect(sitemap).toHaveLength(2);
    });

    it('should handle duplicate slugs', () => {
      const duplicates = [
        { slug: 'same-slug', title: 'Article 1', date: 'Nov 2025', category: 'Test', featured: false },
        { slug: 'same-slug', title: 'Article 2', date: 'Oct 2025', category: 'Test', featured: false },
      ];

      const sitemap = generateSitemap(duplicates);
      expect(sitemap).toHaveLength(3); // homepage + 2 articles (both included)
    });
    ...
    it('should handle articles with future dates', () => {
      ...
      const sitemap = generateSitemap(futureArticle);
      const articleEntry = sitemap.find((e) => e.url.includes('future'));
      expect(articleEntry?.lastModified.getFullYear()).toBe(2099);
    });
    ...
    it('should handle very old dates', () => {
      ...
      const sitemap = generateSitemap(oldArticle);
      const articleEntry = sitemap.find((e) => e.url.includes('old'));
      expect(articleEntry?.lastModified.getFullYear()).toBe(1970);
    });
  });
});
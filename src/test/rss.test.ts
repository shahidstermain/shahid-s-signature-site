...
describe("RSS Feed Generation", () => {
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle articles with no category", () => {
      ...
      vi.spyOn(articlesModule, "articles", "get").mockReturnValue(articlesWithoutCategory as any);

      const rss = generateRSSFeed();
      expect(rss).toContain("<item>");
      expect(rss).toContain("<?xml version");
    });
    ...
    it("should handle content with CDATA-like strings", () => {
      const articlesWithCDATA = [{
        ...mockArticles[0],
        content: "This contains ]]> which looks like CDATA end",
        slug: "cdata-test",
      }];

      vi.spyOn(articlesModule, "articles", "get").mockReturnValue(articlesWithCDATA as any);

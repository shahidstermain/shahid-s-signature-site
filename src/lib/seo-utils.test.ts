...
describe("seo-utils", () => {
  ...
  describe("additional boundary conditions", () => {
    it("should handle URLs with special characters", () => {
      expect(toAbsoluteUrl("/blog/post?utm_source=twitter&utm_campaign=test")).toBe(
        "https://shahidster.tech/blog/post?utm_source=twitter&utm_campaign=test"
      );
    });

    it("should handle URLs with hash fragments", () => {
      expect(toAbsoluteUrl("/docs#section-2")).toBe(
        "https://shahidster.tech/docs#section-2"
      );
    });

    it("should preserve double slashes in absolute URLs", () => {
      const absoluteUrl = "https://example.com//double-slash";
      expect(toAbsoluteUrl(absoluteUrl)).toBe(absoluteUrl);
    });

    it("should handle buildCanonicalUrl with query parameters", () => {
      expect(buildCanonicalUrl("/search?q=test")).toBe(
        "https://shahidster.tech/search?q=test"
      );
    });

    it("should handle very old dates", () => {
      const date = parseArticleDate("Jan 2000");
    ...
    it("should format dates consistently for the same input", () => {
      const date1 = formatArticleDateIso("Mar 2024");
      const date2 = formatArticleDateIso("Mar 2024");
      expect(date1).toBe(date2);
    });

    it("should handle dates at year boundaries", () => {
      const dec31 = parseArticleDate("Dec 2023");
      const jan1 = parseArticleDate("Jan 2024");

      expect(dec31.getTime()).toBeLessThan(jan1.getTime());
    });

    it("should handle path normalization edge cases", () => {
      expect(toAbsoluteUrl("//double-leading-slash")).toBe(
        "https://shahidster.tech//double-leading-slash"
      );
    });

    it("should handle very long paths", () => {
      const longPath = "/a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/p";
      expect(toAbsoluteUrl(longPath)).toBe(
        "https://shahidster.tech" + longPath
      );
    });
  });
});
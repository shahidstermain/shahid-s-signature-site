...
describe("sitemap", () => {
  ...
  describe("additional validation tests", () => {
    it("should not have trailing whitespace in URLs", () => {
      const sitemap = generateSitemap();
      const urls = sitemap.match(/<loc>(.*?)<\/loc>/g);

      urls?.forEach((url) => {
        const extracted = url.replace(/<\/?loc>/g, "");
        expect(extracted).toBe(extracted.trim());
      });
    });

    it("should have consistent date formatting across all entries", () => {
      const sitemap = generateSitemap();
      const dates = sitemap.match(/<lastmod>(.*?)<\/lastmod>/g);

      dates?.forEach((date) => {
        const extracted = date.replace(/<\/?lastmod>/g, "");
        // Should match YYYY-MM-DD format
        expect(extracted).toMatch(/^\d{4}-\d{2}-\d{2}$/);
      });
    });

    it("should have changefreq values from valid set", () => {
      const sitemap = generateSitemap();
      ...
      changefreqs?.forEach((freq) => {
        const extracted = freq.replace(/<\/?changefreq>/g, "");
        expect(validFreqs).toContain(extracted);
      });
    });
    ...
    it("should have priority values between 0 and 1", () => {
      ...
      priorities?.forEach((priority) => {
        const extracted = priority.replace(/<\/?priority>/g, "");
    ...
    it("should not exceed 50MB size limit for sitemaps", () => {
      const sitemap = generateSitemap();
      const sizeInBytes = Buffer.byteLength(sitemap, "utf8");
      const sizeInMB = sizeInBytes / (1024 * 1024);

      // Sitemaps should be under 50MB (actual limit)
      expect(sizeInMB).toBeLessThan(50);
    });

    it("should have properly nested XML elements", () => {
      const sitemap = generateSitemap();

      // Check that URL elements are properly within urlset
      const urlsetStart = sitemap.indexOf("<urlset");
      const urlsetEnd = sitemap.indexOf("</urlset>");
      const firstUrl = sitemap.indexOf("<url>");

      expect(firstUrl).toBeGreaterThan(urlsetStart);
      expect(firstUrl).toBeLessThan(urlsetEnd);
    });

    it("should generate robots.txt with correct line endings", () => {
      const robots = generateRobotsTxt();
      const lines = robots.split("\n");
    ...
    it("should escape special URL characters properly", () => {
      const sitemap = generateSitemap();

      // URLs should not contain unescaped special chars
      expect(sitemap).not.toContain("<loc>http://example.com/path with spaces</loc>");
      expect(sitemap).not.toContain("<loc>http://example.com/path&unescaped</loc>");
    });
  });
});
...
describe("sitemap", () => {
  ...
  describe("edge cases", () => {
    ...
    it("should handle sitemap with maximum URL count", () => {
      const sitemap = generateSitemap();

      // Sitemap should be under 50MB uncompressed (practical limit)
      expect(sitemap.length).toBeLessThan(50 * 1024 * 1024);
    });

    it("should include lastmod for all URLs", () => {
      const sitemap = generateSitemap();
    ...
    it("should format all lastmod dates consistently", () => {
      const sitemap = generateSitemap();

      const dates = sitemap.match(/<lastmod>(\d{4}-\d{2}-\d{2})<\/lastmod>/g);
    ...
    it("should set reasonable priority values", () => {
      const sitemap = generateSitemap();

      const priorities = sitemap.match(/<priority>([\d.]+)<\/priority>/g);
      priorities?.forEach((priority) => {
        const value = parseFloat(priority.match(/[\d.]+/)?.[0] || "0");
    ...
    it("should use valid changefreq values", () => {
      ...
      const changefreqs = sitemap.match(/<changefreq>(.*?)<\/changefreq>/g);
    ...
    it("should escape URLs properly", () => {
      ...
      // Should not contain unescaped ampersands outside of entities
      const locElements = sitemap.match(/<loc>.*?<\/loc>/g);
      locElements?.forEach((loc) => {
        // If it contains &, it should be &amp;
        const hasAmpersand = loc.includes("&");
        if (hasAmpersand) {
          expect(loc).toMatch(/&amp;/);
        }
      });
    ...
    it("should generate valid sitemap for robots.txt reference", () => {
      const sitemap = generateSitemap();
      const robots = generateRobotsTxt();

      // Robots.txt should reference a valid sitemap path
      expect(robots).toContain("/sitemap.xml");
      expect(sitemap).toContain("<?xml");
    });

    it("should handle homepage with higher priority", () => {
      const sitemap = generateSitemap();

      // Homepage should have priority 1.0
      const homepageSection = sitemap.substring(0, sitemap.indexOf("</url>") + 6);
      expect(homepageSection).toContain("<priority>1.0</priority>");
    });

    it("should order URLs appropriately", () => {
      const sitemap = generateSitemap();
    ...
    it("should handle articles with consistent changefreq", () => {
      const sitemap = generateSitemap();

      // All blog posts should have monthly changefreq
      const urls = sitemap.match(/<url>[\s\S]*?<\/url>/g);
      const blogUrls = urls?.filter((url) => url.includes("/blog/"));

      blogUrls?.forEach((url) => {
        expect(url).toContain("<changefreq>monthly</changefreq>");
      });
    });
    ...
    it("should generate robots.txt with proper format", () => {
      const robots = generateRobotsTxt();
    ...
    it("should handle sitemap XML namespace", () => {
      const sitemap = generateSitemap();
    ...
    it("should generate consistent output", () => {
      ...
      // Except for homepage lastmod (current date), structure should be same
      const normalize = (s: string) =>
        s.replace(/<lastmod>\d{4}-\d{2}-\d{2}<\/lastmod>/, "<lastmod>DATE</lastmod>", 1);

      expect(normalize(sitemap1)).toBe(normalize(sitemap2));
    });
  });
});
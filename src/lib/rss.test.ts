...
describe("rss", () => {
  ...
  describe("content sanitization and security", () => {
    it("should properly escape CDATA end sequence in RSS content", () => {
      const rss = generateRSSFeed();
      ...
      // Should not have broken CDATA tags
      expect(rss).not.toContain("]]]]><![CDATA[>");
    });

    it("should handle articles with no keywords in both feeds", () => {
      const rss = generateRSSFeed();
      const feed = generateJSONFeed();
      const parsed = JSON.parse(feed);

      // Article 3 has no keywords
      const article3 = parsed.items.find((i: { title: string }) => i.title === "Recent Article");
      expect(article3.tags).toEqual(["Latest"]); // Only category, no keywords
    });

    it("should maintain URL consistency between RSS and JSON feeds", () => {
      const rss = generateRSSFeed();
      const feed = generateJSONFeed();
      const parsed = JSON.parse(feed);

      // All articles should have consistent URLs
      parsed.items.forEach((item: { url: string; id: string }) => {
        expect(item.url).toBe(item.id);
        expect(rss).toContain(item.url);
      });
    });

    it("should properly format dates for RSS readers", () => {
      const rss = generateRSSFeed();

      // Should have valid RFC 822 dates for all items

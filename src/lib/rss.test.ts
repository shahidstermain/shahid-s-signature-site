...
describe("rss", () => {
  ...
  describe("generateJSONFeed", () => {
    ...
    it("should sort items by date (newest first)", () => {
      ...
      // Should be in reverse chronological order (Mar -> Feb -> Jan)
      expect(parsed.items[0].title).toBe("Test Article One");
      expect(parsed.items[1].title).toContain("Special Characters");
      expect(parsed.items[2].title).toBe("Recent Article");
    });
    ...
    it("should include required item fields", () => {
      ...
      expect(item.title).toBe("Test Article One");
      expect(item.summary).toBe("First test article description");
  ...
  describe("integration between RSS and JSON feeds", () => {
    ...
    it("should have consistent article ordering", () => {
      ...
      // Both should be sorted newest first (Mar before Jan)

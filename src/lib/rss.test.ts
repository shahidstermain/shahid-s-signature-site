...
describe("rss", () => {
  ...
  describe("integration between RSS and JSON feeds", () => {
    ...
    it("should generate valid XML even with one article", () => {
      const rss = generateRSSFeed();
      const json = generateJSONFeed();
    ...
    it("should handle very long content gracefully", () => {
      ...
      expect(rss.length).toBeLessThan(1000000);
      expect(json.length).toBeLessThan(1000000);
    });

    it("should escape CDATA end sequences in content", () => {
      const rss = generateRSSFeed();

      // Should handle ]]> in content without breaking CDATA
      expect(rss).toContain("<![CDATA[");
      expect(rss).toContain("]]>");
    });

    it("should maintain consistent site URL across feeds", () => {
      const rss = generateRSSFeed();
      const json = generateJSONFeed();
      const parsed = JSON.parse(json);

      const siteUrl = "https://shahidster.tech";
      expect(rss).toContain(siteUrl);
      expect(parsed.home_page_url).toBe(siteUrl);
    });

    it("should handle articles with null or undefined fields", () => {
      expect(() => generateRSSFeed()).not.toThrow();
      expect(() => generateJSONFeed()).not.toThrow();
    });

    it("should preserve article order consistency", () => {
      const rss1 = generateRSSFeed();
      const rss2 = generateRSSFeed();

      // Order should be deterministic
      expect(rss1).toBe(rss2);
    });
    ...
    it("should handle unicode characters correctly", () => {
      ...
      // Should preserve unicode in UTF-8 encoding
      expect(rss).toContain('encoding="UTF-8"');
      expect(() => JSON.parse(json)).not.toThrow();
    });

    it("should validate feed structure completeness", () => {
      const rss = generateRSSFeed();
    ...
    it("should validate JSON Feed 1.1 compliance", () => {
      const feed = generateJSONFeed();
      const parsed = JSON.parse(feed);

      // Required JSON Feed 1.1 fields
      expect(parsed.version).toBe("https://jsonfeed.org/version/1.1");
    ...
    it("should handle RSS namespace declarations", () => {
      const rss = generateRSSFeed();
    ...
    it("should generate unique item identifiers", () => {
      const json = generateJSONFeed();
      const parsed = JSON.parse(json);

      const ids = parsed.items.map((item: any) => item.id);
    ...
    it("should format all dates consistently", () => {
      ...
      const dates = rss.match(/<pubDate>.*?<\/pubDate>/g);
      dates?.forEach((date) => {

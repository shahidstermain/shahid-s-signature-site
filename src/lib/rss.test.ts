...
describe("rss", () => {
  ...
  describe("RSS edge cases and validation", () => {
    it("should not contain CDATA end sequence in content", () => {
      const rss = generateRSSFeed();
      // Should not contain ]]> outside of CDATA blocks
      const cdataMatches = rss.match(/<content:encoded><!\[CDATA\[(.*?)\]\]>/gs);
      cdataMatches?.forEach(match => {
        const content = match.match(/<!\[CDATA\[(.*?)\]\]>/s)?.[1];
        expect(content).not.toContain("]]>");
      });
    });

    it("should have valid RFC 822 date format", () => {
      const rss = generateRSSFeed();
      const datePattern = /<pubDate>([^<]+)<\/pubDate>/g;
      const dates = [...rss.matchAll(datePattern)];
    ...
    it("should escape ampersands in all text content", () => {
      const rss = generateRSSFeed();
      // Check that raw ampersands don't exist outside of entity references
      const contentBetweenTags = rss.match(/>([^<]*)</g);
      contentBetweenTags?.forEach(segment => {
        const content = segment.slice(1, -1);
        if (content.includes("&")) {
          // Should only contain valid entities
          expect(content).toMatch(/&(amp|lt|gt|quot|apos);/);
        }
      });
    });

    it("should include lastBuildDate for freshness indication", () => {
      const rss = generateRSSFeed();
      expect(rss).toContain("<lastBuildDate>");
    });

    it("should include atom self-link for feed discovery", () => {
      const rss = generateRSSFeed();
      expect(rss).toContain('<atom:link href=');
      expect(rss).toContain('rel="self"');
    });

    it("should handle articles with undefined seoKeywords", () => {
      const rss = generateRSSFeed();
      expect(rss).toContain("Recent Article");
      // Should not break when article has no seoKeywords
    });

    it("should properly truncate long content in CDATA", () => {
      const rss = generateRSSFeed();
      const cdataMatches = rss.match(/<content:encoded><!\[CDATA\[(.*?)\]\]>/gs);
  ...
  describe("JSON Feed edge cases and validation", () => {
    it("should have valid JSON Feed version", () => {
      const feed = generateJSONFeed();
      const parsed = JSON.parse(feed);

      expect(parsed.version).toMatch(/^https:\/\/jsonfeed\.org\/version\/\d+\.\d+$/);
    });

    it("should include all required feed-level fields", () => {
      const feed = generateJSONFeed();
      const parsed = JSON.parse(feed);

      expect(parsed.version).toBeDefined();
      expect(parsed.title).toBeDefined();
      expect(parsed.home_page_url).toBeDefined();
      expect(parsed.feed_url).toBeDefined();
    ...
    it("should include all required item fields", () => {
      ...
      parsed.items.forEach((item: unknown) => {
        const typedItem = item as Record<string, unknown>;
        expect(typedItem.id).toBeDefined();
        expect(typedItem.url).toBeDefined();
        expect(typedItem.title).toBeDefined();
        expect(typedItem.date_published).toBeDefined();
      });
    });
    ...
    it("should handle articles with very long content", () => {
      ...
      // content_text should be truncated to ~500 chars + "..."
      parsed.items.forEach((item: { content_text: string }) => {
        expect(item.content_text.length).toBeLessThanOrEqual(504);
      });
    });
    ...
    it("should preserve special characters in JSON", () => {
      ...
      // Find the article with special characters
      const specialItem = parsed.items.find((i: { title: string }) =>
        i.title.includes("Special Characters")
      );
    ...
    it("should not include HTML tags in content_text", () => {
      const feed = generateJSONFeed();
      const parsed = JSON.parse(feed);
    ...
    it("should properly flatten tags array", () => {
      const feed = generateJSONFeed();
      const parsed = JSON.parse(feed);

      parsed.items.forEach((item: { tags: unknown[] }) => {
        expect(Array.isArray(item.tags)).toBe(true);

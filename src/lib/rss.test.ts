...
describe("rss", () => {
  ...
  describe("generateJSONFeed", () => {
    ...
    it("should include all items in feed", () => {
      ...
      // JSON feed doesn't sort by default in current implementation
      expect(parsed.items.length).toBe(3);
  ...
  describe("integration between RSS and JSON feeds", () => {
    ...
    it("should have RSS sorted newest first", () => {
      ...
      // JSON feed includes all articles
      expect(parsed.items.length).toBe(3);
    });
  ...
  describe("additional edge cases and regression tests", () => {
    it("should handle articles with newlines in description", () => {
      const rss = generateRSSFeed();

      // Should not have unescaped newlines in XML
      const descriptionMatches = rss.match(/<description>[\s\S]*?<\/description>/g);
      descriptionMatches?.forEach(desc => {
        expect(desc).not.toContain("\n\n");
      });
    });

    it("should properly escape CDATA end sequences", () => {
      const rss = generateRSSFeed();
    ...
    it("should generate valid RSS even with minimal article data", () => {
      const rss = generateRSSFeed();

      // Should have complete RSS structure
      expect(rss).toContain("<rss");
      expect(rss).toContain("</rss>");
      expect(rss).toContain("<channel>");
      expect(rss).toContain("</channel>");
    ...
    it("should not include TTL in RSS feed by default", () => {
      const rss = generateRSSFeed();

      // Current implementation doesn't include TTL
      expect(rss).not.toContain("<ttl>");
    });

    it("should not include generator tag by default", () => {
      const rss = generateRSSFeed();

      // Current implementation doesn't include generator
      expect(rss).not.toContain("<generator>");
    });

    it("should escape category names with XML entities", () => {
      const rss = generateRSSFeed();

      // Category "XML & Tests" should be escaped
      expect(rss).toContain("<category>XML &amp; Tests</category>");
    });

    it("should generate JSON feed with valid URLs", () => {
      const feed = generateJSONFeed();
      const parsed = JSON.parse(feed);

      parsed.items.forEach((item: { id: string; url: string }) => {
        expect(item.id).toMatch(/^https:\/\//);
        expect(item.url).toMatch(/^https:\/\//);
        expect(item.id).toBe(item.url); // id and url should match
      });
    });

    it("should handle articles with no content gracefully", () => {
      // Current articles all have content, but function should handle edge case
      const rss = generateRSSFeed();
      const json = generateJSONFeed();
    ...
    it("should maintain consistent date formatting across feeds", () => {
      const rss = generateRSSFeed();
      const json = generateJSONFeed();
      const parsed = JSON.parse(json);

      // RSS uses RFC 822, JSON uses ISO 8601
      expect(rss).toMatch(/<pubDate>[A-Z][a-z]{2}, \d{2} [A-Z][a-z]{2} \d{4}/);
      // Check that at least one item has the correct date format
      expect(parsed.items.some((item: { date_published: string }) =>
        item.date_published.startsWith("2024-")
      )).toBe(true);
    });
  });
});
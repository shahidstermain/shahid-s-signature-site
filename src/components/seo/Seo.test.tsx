...
describe("Seo component", () => {
  ...
  it("should handle empty string values gracefully", async () => {
    renderWithHelmet(<Seo title="" description="" path="" image="" />);
  ...
  it("should handle undefined vs null props", async () => {
    renderWithHelmet(<Seo title={undefined} description={undefined} />);
  ...
  it("should handle malformed URLs in image prop", async () => {
    renderWithHelmet(<Seo image="not-a-valid-url" />);
  ...
  it("should handle keywords with special characters", async () => {
    renderWithHelmet(<Seo keywords={["test", "keyword-with-dash", "keyword_with_underscore"]} />);

    await waitFor(() => {
      const keywords = document.querySelector('meta[name="keywords"]');
      expect(keywords?.getAttribute("content")).toContain("dash");
    });
  });
  ...
  it("should handle very long keyword arrays", async () => {
    const manyKeywords = Array.from({ length: 100 }, (_, i) => `keyword${i}`);
    renderWithHelmet(<Seo keywords={manyKeywords} />);

    await waitFor(() => {
      const keywords = document.querySelector('meta[name="keywords"]');
      expect(keywords?.getAttribute("content")?.split(",").length).toBe(100);
    });
  });

  it("should not render duplicate meta tags", async () => {
  ...
  it("should handle path with query parameters", async () => {
    renderWithHelmet(<Seo path="/blog?page=2&sort=date" />);

    await waitFor(() => {
      const canonical = document.querySelector('link[rel="canonical"]');
      expect(canonical?.getAttribute("href")).toContain("?page=2");
    });
  });

  it("should handle path with hash fragments", async () => {
    renderWithHelmet(<Seo path="/blog#section" />);

    await waitFor(() => {
      const canonical = document.querySelector('link[rel="canonical"]');
      expect(canonical?.getAttribute("href")).toContain("#section");
    });
  });

  it("should handle JSON-LD with nested objects", async () => {
    const complexJsonLd = {
      "@context": "https://schema.org",
      "@type": "Article",
      author: {
    ...
    renderWithHelmet(<Seo jsonLd={complexJsonLd} />);

    await waitFor(() => {
      const scripts = document.querySelectorAll('script[type="application/ld+json"]');
      expect(scripts.length).toBe(1);
      const content = JSON.parse(scripts[0].textContent || "{}");
      expect(content.author.name).toBe("Test Author");
    });
  });

  it("should handle publishedTime without being article type (graceful degradation)", async () => {
    renderWithHelmet(
      <Seo type="website" publishedTime="2024-01-01T00:00:00.000Z" />
    );
  ...
  it("should sanitize JSON-LD to prevent XSS", async () => {
    ...
    renderWithHelmet(<Seo jsonLd={maliciousJsonLd} />);

    await waitFor(() => {
      const scripts = document.querySelectorAll('script[type="application/ld+json"]');
      const content = scripts[0].textContent || "";
      // JSON.stringify should escape the script tag

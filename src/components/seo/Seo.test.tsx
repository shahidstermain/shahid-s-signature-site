...
describe("Seo component", () => {
  ...
  describe("edge cases and error handling", () => {
    ...
    it("should handle empty arrays in JSON-LD", async () => {
    ...
    it("should properly escape special characters in meta tags", async () => {
      renderWithHelmet(<Seo title='Test "Title" with <special> & chars' />);

      await waitFor(() => {
        const ogTitle = document.querySelector('meta[property="og:title"]');
        expect(ogTitle?.getAttribute("content")).toContain('"');
        expect(ogTitle?.getAttribute("content")).toContain("<");
        expect(ogTitle?.getAttribute("content")).toContain("&");
      });
    });
    ...
    it("should update meta tags when props change", async () => {
      const { rerender } = renderWithHelmet(<Seo title="Initial Title" />);

      await waitFor(() => {
        expect(document.title).toContain("Initial Title");
      });
    ...
    it("should handle rapid re-renders without errors", async () => {
      ...
      await waitFor(() => {
        expect(document.title).toContain("Title 5");
      });
    });
  });
});
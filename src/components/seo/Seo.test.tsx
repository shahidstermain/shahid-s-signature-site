...
describe("Seo component", () => {
  ...
  describe("additional edge cases and regressions", () => {
    ...
    it("should handle special characters in description", async () => {
      const desc = "Description with & tags and quotes";
    ...
    it("should handle relative image URLs", async () => {
      ...
      await waitFor(() => {
        const ogImage = document.querySelector('meta[property="og:image"]');
        expect(ogImage?.getAttribute("content")).toBe("https://shahidster.tech/images/test.png");
      });
    });
    ...
    it("should handle path with query parameters", async () => {
      ...
      await waitFor(() => {
        const canonical = document.querySelector('link[rel="canonical"]');
        expect(canonical?.getAttribute("href")).toBe("https://shahidster.tech/blog?page=2&sort=date");
      });
    });
    ...
    it("should handle path with hash fragment", async () => {
      ...
      await waitFor(() => {
        const canonical = document.querySelector('link[rel="canonical"]');
        expect(canonical?.getAttribute("href")).toBe("https://shahidster.tech/blog#section");
      });
    });

    it("should set og:type to website by default", async () => {
      renderWithHelmet(<Seo />);
    ...
    it("should handle multiple keywords", async () => {
      ...
      await waitFor(() => {
        ...
        expect(keywordsMeta?.getAttribute("content")).toBe("one, two, three, four, five");
      });
    });

    it("should handle keywords with special characters", async () => {
      renderWithHelmet(<Seo keywords={["test debug", "cpp", "key-value"]} />);

      await waitFor(() => {
        const keywords = document.querySelector('meta[name="keywords"]');
        expect(keywords?.getAttribute("content")).toContain("test debug");
        expect(keywords?.getAttribute("content")).toContain("cpp");
      });
    });

    it("should handle ISO 8601 dates", async () => {
      const isoDate = "2024-01-15T10:30:00.000Z";
      renderWithHelmet(<Seo type="article" publishedTime={isoDate} />);

      await waitFor(() => {
        const publishedTime = document.querySelector('meta[property="article:published_time"]');
        expect(publishedTime?.getAttribute("content")).toBe(isoDate);
      });
    });
    ...
    it("should render JSON-LD with nested objects", async () => {
      const jsonLd = {
        "@context": "https://schema.org",
        "@type": "Article",
        author: {
    ...
    it("should preserve og:image when inheriting from parent", async () => {
      renderWithHelmet(<Seo image="/test.png" />);

      await waitFor(() => {
        const ogImage = document.querySelector('meta[property="og:image"]');
        expect(ogImage?.getAttribute("content")).toBe("https://shahidster.tech/test.png");
      });
    });
  });
});
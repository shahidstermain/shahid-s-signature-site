...
describe("Work component", () => {
  ...
  describe("edge cases", () => {
    ...
    it("should display all three companies in correct order", () => {
      const { container } = render(<Work />);

      const articles = Array.from(container.querySelectorAll("article"));
      const companyNames = articles.map(article => article.textContent);
    ...
    it("should handle missing skills gracefully", () => {
      ...
      // All experiences should have skills rendered
      const singlestoreSkills = ["SingleStore", "Distributed SQL", "Linux", "Python"];
      singlestoreSkills.forEach(skill => {
        if (skill === "SingleStore" || skill === "Linux" || skill === "AWS") {
  ...
  describe("brand colors and styling", () => {
    it("should apply different brand colors to each company", () => {
      ...
      // Each article should have unique brand styling
    ...
    it("should render glow effects for each company", () => {
      const { container } = render(<Work />);

      const glowEffects = container.querySelectorAll(".blur-3xl");
      expect(glowEffects.length).toBe(3); // One for each company
    });

    it("should apply hover effects on articles", () => {
      const { container } = render(<Work />);
    ...
    it("should display current badge with brand styling", () => {
      const { container } = render(<Work />);

      const currentBadge = screen.getByText("Current");
      expect(currentBadge).toBeInTheDocument();

      const badge = currentBadge.closest("span");
      expect(badge).toHaveAttribute("style");
    });
  });

  describe("content completeness", () => {
    it("should render all impact items for each company", () => {
      render(<Work />);
    ...
    it("should render all company descriptions", () => {
      render(<Work />);

      expect(screen.getByText(/Resolving Tier-2\/3 distributed systems challenges/i)).toBeInTheDocument();
      expect(screen.getByText(/Delivered technical support for Amazon Aurora, RDS, and AWS DMS/i)).toBeInTheDocument();
      expect(screen.getByText(/Administered SCCM and Windows systems for enterprise clients/i)).toBeInTheDocument();
    });

    it("should display three Key Impact sections", () => {
      render(<Work />);
  ...
  describe("responsive layout tests", () => {
    ...
    it("should have responsive grid layouts for company info", () => {
      const { container } = render(<Work />);

      const articles = container.querySelectorAll("article");
      articles.forEach(article => {
        const flexElements = article.querySelectorAll('[class*="flex"]');
        expect(flexElements.length).toBeGreaterThan(0);
      });
    });

    it("should wrap skill tags", () => {
      const { container } = render(<Work />);

      const skillContainers = container.querySelectorAll('[class*="flex-wrap"]');
      expect(skillContainers.length).toBeGreaterThan(0);
    });
  });
});
...
describe("Hero component", () => {
  ...
  describe("interaction tests", () => {
    it("should navigate to connect section when Let's talk button is clicked", () => {
      ...
      expect(mockGetElementById).toHaveBeenCalledWith("connect");

      mockGetElementById.mockRestore();
    });

    it("should navigate to work section when See my work button is clicked", () => {
      const mockScrollIntoView = vi.fn();
      const mockGetElementById = vi.spyOn(document, "getElementById").mockReturnValue({
        scrollIntoView: mockScrollIntoView,
      } as unknown as HTMLElement);
  ...
  describe("styling and animations", () => {
    it("should have animation delay on glow effects", () => {
      const { container } = render(<Hero />);
    ...
    it("should have correct brand color for SingleStore", () => {
      render(<Hero />);

      // SingleStore branding should be present
      const singlestoreLinks = screen.getAllByText("SingleStore");
      expect(singlestoreLinks.length).toBeGreaterThan(0);
    });

    it("should render with backdrop blur on status badge", () => {
      const { container } = render(<Hero />);
    ...
    it("should render gradient text on heading", () => {
      const { container } = render(<Hero />);

      const gradient = container.querySelector(".text-gradient");
      expect(gradient).toBeInTheDocument();
      expect(gradient).toHaveTextContent("at scale.");
    });
  });

  describe("content validation", () => {
    it("should display correct job description text", () => {
      render(<Hero />);
      ...
      expect(
        screen.getByText(/help Fortune 500 teams ship reliable data infrastructure/i)
      ).toBeInTheDocument();
    });

    it("should render correct number of CTA buttons", () => {
      render(<Hero />);

      const letsTalk = screen.getByText("Let's talk");
      const seeWork = screen.getByText("See my work");
      const resume = screen.getByText("Resume");

      expect(letsTalk).toBeInTheDocument();
    ...
    it("should display SingleStore endorsement card with correct content", () => {
      render(<Hero />);
